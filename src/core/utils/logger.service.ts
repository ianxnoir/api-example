...
import { XTraceDto } from '../decorator/xTraceId.decorator';
...
export class Logger extends BaseLogger {
  ...
  constructor(private configService: ConfigService) {
    ...
    let appDomain = configService.get<any>('logger.appDomain');
    let appService = configService.get<any>('logger.appService');
  ...
  public XTRACE_INFO(xTrace: XTraceDto, message: string, _function: string = '', metadata: any = {}){
    this.INFO(xTrace.xRequestId, "", message, _function, metadata)
  }
  ...
  public getCode(alias: string): string {
    let code = this.codeMapper[alias];
    if (code == undefined) {
      return alias;
    }
